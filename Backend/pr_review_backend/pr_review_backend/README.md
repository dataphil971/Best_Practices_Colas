...
### Lot 6 — Connecteur SharePoint / Excel en ligne

| Méthode | Route | Rôle | Description |
|---|---|---|---|
| GET | `/api/v1/admin/integrations/sharepoint` | admin | Config SharePoint active (secret jamais exposé) |
| PUT | `/api/v1/admin/integrations/sharepoint` | admin | Définir les sources et motifs de fichiers |
| POST | `/api/v1/admin/integrations/sharepoint/sync` | admin | Déclencher une synchro manuelle |

**Microsoft Graph** en OAuth 2.0 client credentials (app Entra ID), permissions
lecture seule (`Sites.Read.All` / `Files.Read.All`). Le client secret vit dans le
coffre (`secret_ref`), jamais en base ni renvoyé au front. Le client Graph est
**tolérant** : sans configuration exploitable il se déclare non opérationnel et la
synchro se dégrade proprement (aucun plantage).

**Sources multiples** configurables par l'admin (`site`, `drive`, `folder`,
`pattern` type `PR_*.xlsx`, `target_checklist`). À la synchro, pour chaque fichier
correspondant au motif : téléchargement, création/mise à jour de la revue cible
(nommée d'après le fichier) et **pré-remplissage via le moteur partagé du Lot 5**
— un seul moteur de parsing + matching, deux sources d'entrée (upload manuel et
SharePoint). Chaque import est tracé comme `import_job` (source=`sharepoint`), avec
**déduplication** : un fichier déjà importé à la même date de modification n'est
pas retraité.

> Le déclencheur de synchro est manuel dans cette livraison ; il se branche tel
> quel sur une planification Celery beat en production.

### Lot 7 — Monitoring, audit & durcissement

| Méthode | Route | Rôle | Description |
|---|---|---|---|
| GET | `/health` | public | Liveness (l'app répond) |
| GET | `/health/ready` | public | Readiness (vérifie la base de données) |
| GET | `/api/v1/monitoring/portfolio` | auth | Agrégats portefeuille (scores, bloquants, avancement), filtrables `?type=` `?status=` |
| GET | `/api/v1/monitoring/reviews/{id}` | auth+accès | Détail d'avancement (par catégorie, par remédiation) |
| GET | `/api/v1/admin/settings` | admin | Lire les paramètres (dont `retention_days`, `role_mapping`) |
| PUT | `/api/v1/admin/settings/retention` | admin | Modifier la rétention (jours) |
| PUT | `/api/v1/admin/settings/role-mapping` | admin | Mapper des groupes Entra ID → rôles |
| POST | `/api/v1/admin/settings/retention/purge` | admin | Déclencher la purge de rétention |
| GET | `/api/v1/admin/audit` | admin | Consulter le journal d'audit (filtrable) |

**Audit immuable** — `audit_log` trace toute action sensible (approbation de règle,
validation de revue, changement de rôle, configuration de connecteur, purge) avec
acteur, entité, IP et horodatage. L'immuabilité est garantie en base par un
**trigger** interdisant `UPDATE`/`DELETE`. On ne journalise jamais de contenu
d'entreprise — seulement le fait.

**Rate limiting** — limiteur à fenêtre glissante sur les points sensibles :
`/auth/login/local` (10/min), `/auth/register` (5/min), `/reviews/{id}/import-answers`
(10/min). Contrat prêt à basculer sur Redis en multi-instances.

**Rétention** — purge quotidienne (Celery beat en prod ; déclenchable manuellement)
supprimant les fichiers importés et `import_jobs` au-delà de `retention_days`
(défaut 30, réglable). La suppression passe par `StorageProvider.delete()` — quel
que soit le backend. **Les revues ne sont jamais purgées** : seule la matière
première d'import l'est.

**Monitoring** — le portefeuille agrège nombre de revues, répartition par statut,
score moyen, points bloquants ouverts (KO de criticité `blocking`) et avancement
des remédiations, dans le respect des règles de visibilité des lots précédents.
...
(pré-remplissage + capture d'échec). Le Lot 6 ajoute `tests/test_sharepoint.py` :
porte opérationnelle du client Graph, filtrage par motif, synchro complète avec
Graph simulé (création + pré-remplissage de la revue cible via le moteur partagé)
et déduplication des fichiers inchangés. Le Lot 7 ajoute `tests/test_monitoring.py` :
écriture d'audit, paramètres (rétention, mapping de rôles filtré), purge de
rétention (fichiers + jobs expirés, revues préservées), agrégats de monitoring
(portefeuille, avancement par catégorie) et limiteur à fenêtre glissante.
**42 tests au total, tous verts.**
