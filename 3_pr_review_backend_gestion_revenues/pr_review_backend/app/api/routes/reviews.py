...
def _assert_can_view(review: Review, user: User) -> None:
    if user.role == UserRole.admin or review.author_id == user.id:
        return
    # Le partage ciblé (Lot 4) élargira cette visibilité aux reviewers désignés.
...
def list_reviews(
    ...
    - `user` → uniquement les siennes ; `admin` → toutes.
