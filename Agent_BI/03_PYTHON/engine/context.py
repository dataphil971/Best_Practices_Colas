...
def _compute_fingerprint(tables: List[TableDef], extra_files: List[Path]) -> Optional[str]:
    ...
    for path in paths:
        stat = path.stat()
        hasher.update(f"{path}:{stat.st_size}:{stat.st_mtime_ns}".encode("utf-8"))
...
class AnalysisContext:
    def __init__(self, project_path: Optional[Path] = None):
        ...
        self.relationships_tmdl_path: Optional[Path] = None
        self.model_tmdl_path: Optional[Path] = None
        self.expressions_tmdl_path: Optional[Path] = None
    ...
    def _attach_report(self, project_path: Path) -> None:
        ...
        self.report_filters = parse_report_filters(self.report_path)
        self.report_field_references = parse_report_field_references(self.report_path)
        self.report_implicit_aggregations = parse_report_implicit_aggregations(self.report_path)
        self.report_structure = parse_report_structure(self.report_path)
        self.report_visuals = parse_report_visuals(self.report_path)
    ...
    def load(cls, project_path: Union[str, Path]) -> "AnalysisContext":
        ...
        if project_path.name.endswith(".SemanticModel") and (
            project_path / "definition" / "tables"
        ).exists():
