...
def check(context: AnalysisContext) -> RuleResult:
    ...
    def _record(status_reason, object_type, object_name, evidence, location=None,
                remediation=""):
        if status_reason is None:
            findings.append(Finding(
                rule_id=RULE_ID, object_type=object_type, object=object_name,
                expected="convention de nommage respectée", actual=None,
                status="OK", evidence=evidence, location=location,
            ))
        else:
            ...
            ko_details.append({
                "object_type": object_type, "object_name": object_name, "reason": reason,
            })
    ...
    for table in context.tables:
        ...
        _record(
            _check_table_name(table.name), "table", table.name, evidence,
        ...
        for column in table.columns:
            ...
            _record(
                _check_column_name(column.name), "column", object_name, evidence,
            ...
            if folder is not None and str(folder) != "":
                folders_by_table.setdefault(table.name, {}).setdefault(
                    str(folder).lower(), []
                ).append((str(folder), object_name))

        for measure in table.measures:
            total_measures += 1
            object_name = f"{table.name}.{measure.raw_name}"
            evidence = {
                "table": table.name, "measure": measure.raw_name,
                "source_file": measure.source_file,
            }
            _record(
                _check_measure_name(measure.name), "measure", object_name, evidence,
        ...
        for normalized, entries in groups.items():
            ...
            for raw, object_name in entries:
                ...
                if whitespace_issue is not None:
                    ko_details.append({
                        "object_type": "displayFolder", "object_name": f"{table_name}::{object_name}",
                        "reason": whitespace_issue[1],
                    })
                    findings.append(Finding(
                        rule_id=RULE_ID, object_type="displayFolder",
                        object=f"{table_name}::{object_name}",
                        expected="displayFolder sans espace superflu, casse cohérente",
                        actual=raw, status="KO", reason=whitespace_issue[1],
                    ...
                    findings.append(Finding(
                        ...
                        actual=raw, status="KO", reason=reason,
                        evidence={"table": table_name, "display_folder": raw, "variants": sorted(distinct_raw)},
                    ))
